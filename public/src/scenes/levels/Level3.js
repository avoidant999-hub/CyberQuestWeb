...
export class Level3 extends Phaser.Scene {
  ...
  createHeader(width, height) {
    ...
    // --- TOMBOL KELUAR ---
    const btnExit = this.add.container(80, headerH/2);
    const bg = this.add.graphics().fillStyle(THEME.colors.card, 1).fillRoundedRect(-50, -20, 100, 40, 10)
                 .lineStyle(1, 0xffffff, 0.2).strokeRoundedRect(-50, -20, 100, 40, 10);
    const txt = this.add.text(0, 0, "KEMBALI", { fontFamily: THEME.fonts.header, fontSize: "12px", fontStyle:"bold" }).setOrigin(0.5);
    
    btnExit.add([bg, txt])
           .setInteractive(new Phaser.Geom.Rectangle(-50, -20, 100, 40), Phaser.Geom.Rectangle.Contains)
  ...
  finishLevel() {
    ...
    const btnTxt = this.add.text(0, 0, "KEMBALI KE MENU", { fontFamily: "Poppins", fontSize: "14px", fontStyle:"bold" }).setOrigin(0.5);
